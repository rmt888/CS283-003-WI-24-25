...
int process_cli_requests(int svr_socket){
    ...
    while(1){
        ...
        int client_rc = exec_client_requests(cli_socket);
        if (client_rc < 0) {
            rc = client_rc;
            break;
        }
...
int exec_client_requests(int cli_socket) {
    ...
    while(1) {
        ...
        ssize_t bytes_received = recv(cli_socket, io_buff + total_bytes_received, buffer_size - total_bytes_received, 0);
        if (bytes_received == -1) {
            perror("recv");
            free(io_buff);
            return;
        ...
        command_list_t cmd_list;
        int parse_rc = parse_commands(io_buff, &cmd_list);
        if (parse_rc != OK) {
            free(io_buff);
            return;
        }
        ...
        for (int i = 0; i < cmd_list.num; i++) {
            int cmd_rc = rsh_execute_pipeline(&cmd_list.commands[i]);
            if (cmd_rc != OK) {
                free(io_buff);
                return;
            }
            ...
            snprintf(response_buffer, sizeof(response_buffer), "Command output: %s", command_output);
            int send_rc = send_message_string(cli_socket, response_buffer);
            if (send_rc != OK) {
                free(io_buff);
                return;
            }
            //send EOF character to indicate end of message
            int eof_rc = send_message_eof(cli_socket);
            if (eof_rc != OK) {
                free(io_buff);
                return; //Exit the function if sending EOF fails
            }
            //handle 'exit' and 'stop-server' commands
            if (strcmp(cmd_list.commands[i][0], "exit") == 0) {
                free(io_buff);
                return;
            } else if (strcmp(cmd_list.commands[i][0], "stop-server") == 0) {
                free(io_buff);
                return;
            }
        }
...
int send_message_string(int cli_socket, char *buff) {
    ...
    size_t message_length = strlen(buff);
    while (total_bytes_sent < message_length) {
        ssize_t bytes_sent = send(cli_socket, buff + total_bytes_sent, message_length - total_bytes_sent, 0);
        if (bytes_sent == -1) {
            perror("send");
            return ERR_RDSH_COMMUNICATION;
        }
...
int rsh_execute_pipeline(int cli_sock, command_list_t *clist) {
    ...
    for (int i = 0; i < clist->num; i++) {
        ...
        if (pids[i] == -1) {
            ...
            //execute the command
            execvp(clist->commands[i][0], clist->commands[i]);
            perror("execvp");
            exit(EXIT_FAILURE);
        }
    }
