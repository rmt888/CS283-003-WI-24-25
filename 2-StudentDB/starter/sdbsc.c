...
int get_student(int fd, int id, student_t *s) {
    ...
    while (1) {
        ...
        //reads the student record from the file
        ssize_t bytes_read = read(fd, &temp_student, STUDENT_RECORD_SIZE);
        if (bytes_read == -1) {
            return ERR_DB_FILE;
        }
        //the student does not exist if there is not match and it reaches the end of the file
        if (bytes_read == 0) {
            return SRCH_NOT_FOUND;
        }
        //checks if the student id matches
        if (temp_student.id == id) {
            //if the id matches then the student is found and it copies data into the pointer
            memcpy(s, &temp_student, STUDENT_RECORD_SIZE);
            return NO_ERROR;
        }
    }
}
...
int add_student(int fd, int id, char *fname, char *lname, int gpa) {
    ...
    //validation
    if (fd < 0 || fname == NULL || lname == NULL || gpa < MIN_STD_GPA || gpa > MAX_STD_GPA || id < MIN_STD_ID || id > MAX_STD_ID) {
        return ERR_DB_FILE;
    }
    ...
    new_student.gpa = gpa;
    //traverse the database to check for duplicates and find an empty spot
    off_t position = 0;
    while (read(fd, &temp_student, STUDENT_RECORD_SIZE) == STUDENT_RECORD_SIZE) {
        if (temp_student.id == id) {
            //student already exists aka no duplicated allowed
            printf("Cant add student with ID=%d, already exists in db.\n", id);
            return ERR_DB_OP;
        }
        if (memcmp(&temp_student, &empty_student, STUDENT_RECORD_SIZE) == 0) {
            //empty spot found so it writes the new student here
        ...
        position += STUDENT_RECORD_SIZE;
    }
    //append the new student at the end of the file
...
int del_student(int fd, int id) {
    student_t s;
    int index = (id - MIN_STD_ID) * STUDENT_RECORD_SIZE;
...
int count_db_records(int fd) {
    ...
    //reads the file in chunks of STUDENT_RECORD_SIZE
    while ((bytes_read = read(fd, &temp_student, STUDENT_RECORD_SIZE)) > 0) {
        //check if the read bytes match the size of a student record
        if (bytes_read != STUDENT_RECORD_SIZE) {
            return ERR_DB_FILE;
        }
        //if the student record is not empty it increments the count
...
int print_db(int fd) {
    ...
    while ((bytes_read = read(fd, &student, STUDENT_RECORD_SIZE)) > 0) {
        ...
        if (memcmp(&student, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0) {
            //if it's the first valid record
            if (!found) {
...
void print_student(student_t *s) {
    //checks if the student pointer is NULL or if the student's ID is 0
    if (s == NULL || s->id == 0) {
    ...
    //calculate the real GPA
    float gpa = s->gpa / 100.0;
    //print the student's data
    printf("%d %-24s %-32s %.2f\n", s->id, s->fname, s->lname, gpa);
}
